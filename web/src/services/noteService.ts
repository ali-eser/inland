...
const createNote = async (note: NewNote) => {
  try {
    ...
    return { error: 'An error occurred while creating a note.' }
  }
}
...
const deleteNote = async (id: number) => {
  try {
    ...
    if (!response.ok) {
      const errorJson = await response.json();
      return { error: errorJson.error || "An unspecified error occurred while deleting." };
    }
...
export default { fetchNotes, updateNote, createNote, deleteNote };
